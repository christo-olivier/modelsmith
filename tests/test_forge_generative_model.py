...
def test_forge_few_shot_text_model_pydantic_model(
    generative_model: GenerativeModel, model_settings: dict[str, Any]
) -> None:
    """
    This test ensures the Forge works as expected with a GenerativeModel when trying
    ...
    """)
    forge = Forge(model=generative_model, response_model=User)
